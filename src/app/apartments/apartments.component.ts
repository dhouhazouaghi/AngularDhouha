...
export class ApartmentsComponent {
  ...
  searchSurface: number = 0;

  getApartmentsByResidence() {
    return this.listApartments.filter(ap => 
      ap.residence.id == this.idResidence &&
      (this.searchSurface ? ap.surface >= this.searchSurface : true)
    );
  }

  favoriteApartments: number[] = [];

  addToFavorites(apartmentId: number): void {
  if (!this.favoriteApartments.includes(apartmentId)) {
    this.favoriteApartments.push(apartmentId);
  }
